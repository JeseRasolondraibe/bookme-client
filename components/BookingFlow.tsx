    ...
    <main className="min-h-screen bg-stone-50 flex flex-col items-center py-10 px-4">
      <div className="w-full max-w-md">
        <header className="flex items-center gap-4 mb-8">
          <div className="w-14 h-14 rounded-full bg-accent-50 flex items-center justify-center text-accent-600 font-semibold text-lg flex-shrink-0 overflow-hidden">
            {presta.avatar_url
        ...
        {/* 3 étapes : prestation → créneau → contact. La confirmation est un écran de résultat, pas une étape à progresser. */}
        {step < 4 && (
          <div className="flex gap-1.5 mb-8">
            {[1,2,3].map(n => (
              <div key={n} className={`h-1 flex-1 rounded-full transition-colors duration-300 ${n <= step ? "bg-accent-600" : "bg-stone-200"}`} />
            ))}
          </div>
        )}
